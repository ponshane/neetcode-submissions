class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        rowCnt = defaultdict(set)
        columnCnt = defaultdict(set)
        subboxCnt = defaultdict(set)
        ...
        for r in range(9):
            for c in range(9):
                ...
                if (num in rowCnt[r] or num in columnCnt[c] or num in subboxCnt[(r // 3, c // 3)]):
                    return False
                else:
                    rowCnt[r].add(num)
                    columnCnt[c].add(num)
                    subboxCnt[(r // 3, c // 3)].add(num)
        return True
